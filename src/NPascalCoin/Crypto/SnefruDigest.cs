...
{
    public class SnefruDigest
        : IDigest, IMemoable
    {
        #region Consts

        private static readonly int[] SShifts = {16, 8, 16, 24};

        private static readonly uint[][] SBoxes =
        {
            new uint[]
            {
        ...
        private void Finish()
        {
            ...
            if (_bufferPos > 0)
            {
                padIndex = (2 * GetByteLength()) - _bufferPos - 8;
            }
            else
            {
                padIndex = GetByteLength() - _bufferPos - 8;
            }
        ...
        private void ProcessBlock(ref uint[] block)
        {
            for (int i = 0; i < _rounds; i++)
            {
                uint[] sBox0 = SBoxes[i * 2];
                uint[] sBox1 = SBoxes[i * 2 + 1];

                for (int j = 0; j < 4; j++)
                {
                    block[15] ^= sBox0[(byte) block[0]];
                    block[1] ^= sBox0[(byte) block[0]];
                    block[0] ^= sBox0[(byte) block[1]];
                    block[2] ^= sBox0[(byte) block[1]];
                    block[1] ^= sBox1[(byte) block[2]];
                    block[3] ^= sBox1[(byte) block[2]];
                    block[2] ^= sBox1[(byte) block[3]];
                    block[4] ^= sBox1[(byte) block[3]];
                    block[3] ^= sBox0[(byte) block[4]];
                    block[5] ^= sBox0[(byte) block[4]];
                    block[4] ^= sBox0[(byte) block[5]];
                    block[6] ^= sBox0[(byte) block[5]];
                    block[5] ^= sBox1[(byte) block[6]];
                    block[7] ^= sBox1[(byte) block[6]];
                    block[6] ^= sBox1[(byte) block[7]];
                    block[8] ^= sBox1[(byte) block[7]];
                    block[7] ^= sBox0[(byte) block[8]];
                    block[9] ^= sBox0[(byte) block[8]];
                    block[8] ^= sBox0[(byte) block[9]];
                    block[10] ^= sBox0[(byte) block[9]];
                    block[9] ^= sBox1[(byte) block[10]];
                    block[11] ^= sBox1[(byte) block[10]];
                    block[10] ^= sBox1[(byte) block[11]];
                    block[12] ^= sBox1[(byte) block[11]];
                    block[11] ^= sBox0[(byte) block[12]];
                    block[13] ^= sBox0[(byte) block[12]];
                    block[12] ^= sBox0[(byte) block[13]];
                    block[14] ^= sBox0[(byte) block[13]];
                    block[13] ^= sBox1[(byte) block[14]];
                    block[15] ^= sBox1[(byte) block[14]];
                    block[14] ^= sBox1[(byte) block[15]];
                    block[0] ^= sBox1[(byte) block[15]];

                    int shift = SShifts[j];

                    for (int k = 0; k < block.Length; k++)
                        block[k] = RotateRight32(block[k], shift);
                }
            }
        ...
        private void ProcessFilledBuffer()
        {
            ...
            int idx = _state.Length;
            for (int i = 0; i < 8; i++, idx++)
            {
                work[idx] = Pack.BE_To_UInt32(_buffer, i * 4);
            }
        ...
        */
        protected SnefruDigest(int rounds, int digestLength, int byteLength)
        {
            _rounds = rounds;
            _digestLength = digestLength;
            _byteLength = byteLength;
            _state = new uint[GetDigestSize() / 4];
         ...
         * message digest.
         */
        public SnefruDigest(SnefruDigest t)
        {
            Reset(t);
        }
        ...
        {
            get { return "Snefru_" + _rounds + "_" + (GetDigestSize() * 8); }
        }
        ...
        public int DoFinal(byte[] output, int outOff)
        {
            ...
            Pack.UInt32_To_BE(_state, output, outOff);
        ...
        {
            return new SnefruDigest(this);
        }

        public void Reset(IMemoable other)
        {
            SnefruDigest originalDigest = (SnefruDigest) other;

            _state = new uint[originalDigest._state.Length];
    ...
    public class Snefru8_256Digest : SnefruDigest
    {
        /**
        * Standard Snefru8_256 constructor
        */
        public Snefru8_256Digest()
            : base(8, 32, 32)
        {
        }
    }
